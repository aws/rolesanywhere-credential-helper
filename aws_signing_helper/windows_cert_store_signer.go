...
func GetMatchingCertsAndChain(certIdentifier CertIdentifier) (store C.HCERTSTORE, pcCertChain []C.PCCERT_CONTEXT, matchingCerts []*x509.Certificate, err error) {
	...
	for {
		...
		if certMatches(certIdentifier, *cert) {
			...
			// We only care about this variable when just a single certificate matches the CertIdentifier
			if pcCertChain == nil {
...
func GetCertStoreSigner(certIdentifier CertIdentifier) (signer Signer, signingAlgorithm string, err error) {
	...
	} else if len(matchingCerts) == 0 {
		return nil, "", errors.New("no matching certs found in cert store")
	}
...
func (signer WindowsCertStoreSigner) Sign(rand io.Reader, digest []byte, opts crypto.SignerOpts) ([]byte, error) {
	...
	if privateKey.capiProv != 0 {
		return signer.capiSignHash(opts.HashFunc(), digest)
	} else if privateKey.cngHandle != 0 {
...
func (signer WindowsCertStoreSigner) cngSignHash(hash crypto.Hash, digest []byte) ([]byte, error) {
	...
	if err := checkStatus(C.NCryptSignHash(privateKey.cngHandle, padPtr, digestPtr, digestLen, nil, 0, &sigLen, flags)); err != nil {
	...
	sigPtr := (*C.BYTE)(&sig[0])
	if err := checkStatus(C.NCryptSignHash(privateKey.cngHandle, padPtr, digestPtr, digestLen, sigPtr, sigLen, &sigLen, flags)); err != nil {
...
func (signer WindowsCertStoreSigner) capiSignHash(hash crypto.Hash, digest []byte) ([]byte, error) {
	...
	privateKey, _ := signer.getPrivateKey()
	if ok := C.CryptCreateHash(C.HCRYPTPROV(privateKey.capiProv), hash_alg, 0, 0, &chash); ok == winFalse {
	...
	if ok := C.CryptSignHash(chash, privateKey.keySpec, nil, 0, nil, &sigLen); ok == winFalse {
	...
	if ok := C.CryptSignHash(chash, privateKey.keySpec, nil, 0, sigPtr, &sigLen); ok == winFalse {
